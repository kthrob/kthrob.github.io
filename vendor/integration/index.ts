  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('promethic');

        const virtualModuleId = 'promethic:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-promethic-config',
                resolveId(id) {
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`Promethic \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`Promethic Systems PWA config loaded successfully.`);
        }
      },
